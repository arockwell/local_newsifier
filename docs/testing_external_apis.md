...
The test mode pattern allows components to automatically detect when they're running in a test environment and provide mock responses without making real API calls.
...
1. **Isolate External Dependencies**
   - Always use mocks or test mode for unit tests
   - Separate true integration tests with clear markers
...
3. **Implement Test Mode**
   - Design services with easy test mode detection
