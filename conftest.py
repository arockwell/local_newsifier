...
# Import and register models in a specific order - imports needed for metadata registration
# flake8: noqa F401
...
def test_engine(request):
    ...
    # Create database URL for each worker (for xdist) or use shared memory
    if worker_id:
...
async def async_test_engine(request):
    """Create an async test database engine using SQLite in-memory.

    This fixture creates an async SQLite in-memory database for testing
    async operations. It follows the same pattern as the sync test_engine
    but uses async SQLAlchemy components.
    """
    # Detect whether we're running with xdist and get the worker ID
    worker_id = getattr(request.config, "workerinput", {}).get("workerid", None)

    # Create a unique database URL for each worker
    if worker_id:
    ...
    async with async_sessionmaker_local() as session:
        ...
        result = await session.execute(
            sa_select(Article).where(Article.title == "Test Article Async")
        )
        article = result.scalar_one_or_none()

        if not article:
            pytest.fail("Failed to verify async table creation")
...
async def async_db_session(async_test_engine) -> AsyncGenerator[AsyncSession, None]:
    """Provide an async database session with transaction isolation.

    This fixture:
    1. Creates a new async connection to the database
    2. Starts a transaction
    3. Yields an async session bound to that connection
    4. Rolls back the transaction after the test
    5. Closes the connection

    This ensures that async tests are isolated from each other.
    """
    async_sessionmaker_local = async_sessionmaker(
