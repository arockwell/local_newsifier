...
def upgrade() -> None:
    ...
    op.execute(
        "ALTER INDEX ix_feed_processing_log_feed_id "
        "RENAME TO ix_rss_feed_processing_logs_feed_id"
    )
    op.execute(
    ...
    op.execute(
        "ALTER TABLE rss_feed_processing_logs "
        "ADD CONSTRAINT rss_feed_processing_logs_feed_id_fkey "
...
def downgrade() -> None:
    ...
    op.execute(
        "ALTER TABLE rss_feed_processing_logs "
        "DROP CONSTRAINT rss_feed_processing_logs_feed_id_fkey"
    )
    op.execute(
        "ALTER INDEX ix_rss_feed_processing_logs_feed_id "
        "RENAME TO ix_feed_processing_log_feed_id"
    )
    op.execute(
    ...
    op.execute(
        "ALTER TABLE feed_processing_log "
        "ADD CONSTRAINT feed_processing_log_feed_id_fkey "
