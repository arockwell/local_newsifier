...
# No type imports - we'll use Any for annotations
# to avoid circular dependencies and prevent import errors
...
def get_opinion_visualizer_tool(session: Annotated[Session, Depends(get_session)]):
    ...
    # Create the instance using the standardized pattern
    tool = OpinionVisualizerTool()
...
@injectable(use_cache=False)
def get_trend_reporter_tool(file_writer: Annotated[Any, Depends(get_file_writer_tool)]):
    ...
    Args:
        file_writer: File writer tool for saving reports
...
def get_entity_extractor():
    ...
    """
    from local_newsifier.tools.extraction.entity_extractor import \
        EntityExtractor
...
def get_apify_schedule_manager(
    ...
    """
    from local_newsifier.services.apify_schedule_manager import \
        ApifyScheduleManager
...
def get_news_pipeline_service(
    ...
    """
    from local_newsifier.services.news_pipeline_service import \
        NewsPipelineService
...
def get_headline_trend_flow(
    ...
    """
    from local_newsifier.flows.analysis.headline_trend_flow import \
        HeadlineTrendFlow
