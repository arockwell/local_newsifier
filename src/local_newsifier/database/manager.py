...
from ..models.database import (
    ...
    Article, ArticleCreate, Entity, EntityCreate,
...
class DatabaseManager:
    ...
    def get_article(self, article_id: int) -> Optional[Article]:
        ...
        """
        db_article = self.session.query(ArticleDB).filter(ArticleDB.id == article_id).first()
    ...
    def get_articles_by_status(self, status: str) -> List[Article]:
        ...
        """
        db_articles = self.session.query(ArticleDB).filter(ArticleDB.status == status).all()
    ...
    def get_entities_by_article(self, article_id: int) -> List[Entity]:
        ...
        """
        db_entities = self.session.query(EntityDB).filter(EntityDB.article_id == article_id).all()
    ...
    def get_analysis_results_by_article(self, article_id: int) -> List[AnalysisResult]:
        ...
        """
        db_results = self.session.query(AnalysisResultDB).filter(AnalysisResultDB.article_id == article_id).all()
