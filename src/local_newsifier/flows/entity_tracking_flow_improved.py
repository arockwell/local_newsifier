...
class EntityTrackingFlow(Flow):
    ...
    def _ensure_dependencies(self):
        ...
        if self._entity_extractor is None and self.container:
            self._entity_extractor = self.container.get("entity_extractor_tool")
            
        if self._context_analyzer is None and self.container:
            self._context_analyzer = self.container.get("context_analyzer_tool")
            
        if self._entity_resolver is None and self.container:
            self._entity_resolver = self.container.get("entity_resolver_tool")
        
        # Service factory
        if self._session_factory is None and self.container:
            self._session_factory = self.container.get("session_factory")
        elif self._session_factory is None:
            # Default fallback if no container or direct dependency
            self._session_factory = SessionManager
        
        # Entity service - requires more dependencies
        if self.entity_service is None:
            # Check container first
            if self.container:
                self.entity_service = self.container.get("entity_service")
            
            # If still None, create with available dependencies
            if self.entity_service is None and all([
                self._entity_extractor, self._context_analyzer, self._entity_resolver
            ]):
    ...
    def process(self, state: EntityTrackingState) -> EntityTrackingState:
        ...
        """
        # Ensure dependencies before processing
        self._ensure_dependencies()
