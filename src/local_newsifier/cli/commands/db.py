...
def check_duplicates(limit: int, json_output: bool):
    ...
    for url, count in duplicate_urls:
        ...
        for article in duplicates:
            duplicate_info["articles"].append({
                "id": article.id,
                "title": article.title[:50] + "..." if article.title and len(article.title) > 50 else article.title,
    ...
    click.echo(click.style(f"Found {len(results)} URLs with duplicate articles:", fg="yellow", bold=True))
...
@click.option("--json", "json_output", is_flag=True, help="Output as JSON")
def list_articles(source: Optional[str], status: Optional[str], 
    ...
    if before:
        try:
            ...
            click.echo(click.style("Error: Invalid date format for --before. Use YYYY-MM-DD", fg="red"), err=True)
            return
    ...
    if after:
        try:
            ...
            click.echo(click.style("Error: Invalid date format for --after. Use YYYY-MM-DD", fg="red"), err=True)
            return
...
def inspect_record(table: str, id: int, json_output: bool):
    ...
    if table == "article":
        ...
        result = {
            ...
            "last_fetched_at": format_datetime(feed.last_fetched_at) if feed.last_fetched_at else None,
...
def purge_duplicates(dry_run: bool, json_output: bool):
    ...
    # Display in human-readable format
    click.echo(click.style(f"{action_text} {total_removed} duplicate articles across {len(results)} URLs", 
                          fg="green" if not dry_run else "yellow", bold=True))
