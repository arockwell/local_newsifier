...
class EntityService:
    ...
    @handle_database
    @monitor_entity_extraction(source="article_processing")
    def process_article_entities(
        self, article_id: int, content: str, title: str, published_at: datetime
    ) -> List[Dict[str, Any]]:
        ...
        with self.session_factory() as session:
            ...
            existing_entities = [
                {"name": entity.name, "entity_type": entity.entity_type, "id": entity.id}
            ...
            for entity in entities:
                ...
                resolved_entity = self.entity_resolver.resolve_entity(
                    entity["text"], entity["type"], existing_entities
                )
    ...
    def generate_entity_dashboard(self, state: EntityDashboardState) -> EntityDashboardState:
        ...
        try:
            ...
            with self.session_factory() as session:
                ...
                entities = self.canonical_entity_crud.get_by_type(
                    session, entity_type=state.entity_type
                )
                ...
                for entity in entities:
                    # Get mention count
                    mention_count = self.canonical_entity_crud.get_mentions_count(
    ...
    def find_entity_relationships(self, state: EntityRelationshipState) -> EntityRelationshipState:
        ...
        try:
            ...
            state.status = TrackingStatus.PROCESSING
            state.add_log(
                f"Finding relationships for entity {state.entity_id} over past {state.days} days"
            )
            ...
            with self.session_factory() as session:
                ...
                for article in articles:
                    ...
                    article_entities = self.entity_crud.get_by_article(
