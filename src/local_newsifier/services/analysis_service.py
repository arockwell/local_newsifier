...
class AnalysisService:
    ...
    def analyze_headline_trends(
        self, start_date: datetime, end_date: datetime, time_interval: str = "day", top_n: int = 20
    ) -> Dict[str, Any]:
        ...
        with self.session_factory() as session:
            ...
            result = {
                ...
                "raw_data": trend_data,
                "period_counts": {
                    period: len(headlines) for period, headlines in grouped_headlines.items()
                },
            }
    ...
    def _get_headlines_by_period(
        self, session: Session, start_date: datetime, end_date: datetime, interval: str = "day"
    ) -> Dict[str, List[str]]:
    ...
    def detect_entity_trends(
        ...
        with self.session_factory() as session:
            ...
            entities = self.entity_crud.get_by_date_range_and_types(
                session, start_date=start_date, end_date=end_date, entity_types=entity_types
            )
    ...
    def _save_analysis_result(
        self, session: Session, article_id: int, analysis_type: str, results: Dict[str, Any]
    ) -> AnalysisResult:
        ...
        existing = self.analysis_result_crud.get_by_article_and_type(
            session, article_id=article_id, analysis_type=analysis_type
        )
        ...
        if existing:
            ...
            new_result = AnalysisResult(
                article_id=article_id, analysis_type=analysis_type, results=results
            )
