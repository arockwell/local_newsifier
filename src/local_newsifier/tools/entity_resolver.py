...
class EntityResolver:
    ...
    def __init__(self, db_manager: DatabaseManager, similarity_threshold: float = 0.85):
        ...
        self.common_titles = [
            ...
            "chief", "ceo", "cto", "cfo", "director", "chairman", "chairwoman",
            "judge", "justice", "secretary", "minister", "sir", "madam"
        ]
        self.name_patterns = {
            # Standard name with titles: "President Joe Biden" -> "Joe Biden"
            r"^(?:{})\s+([A-Z][a-z]+(?:\s+[A-Z][a-z]+)+)$".format("|".join(self.common_titles)): 
            lambda m: m.group(1),
            ...
            r"^([A-Z][a-z]+(?:\s+[A-Z][a-z]+)+)\s+(?:Jr\.|Sr\.|II|III|IV|V)$": 
            lambda m: m.group(1),
        }
    ...
    def find_matching_entity(self, name: str, entity_type: str = "PERSON") -> Optional[CanonicalEntity]:
        """
        ...
        canonical_entity = self.db_manager.get_canonical_entity_by_name(name, entity_type)
        if canonical_entity:
            return canonical_entity
        
        # If no exact match, try normalized match
        normalized_name = self.normalize_entity_name(name)
        canonical_entity = self.db_manager.get_canonical_entity_by_name(normalized_name, entity_type)
        if canonical_entity:
            return canonical_entity
        
        # If still no match, search for similar entities
        try:
            all_canonical_entities = self.db_manager.get_all_canonical_entities(entity_type)
    ...
    def resolve_entity(self, name: str, entity_type: str = "PERSON", metadata: Dict = None) -> CanonicalEntity:
        """
        ...
        if not canonical_entity:
            ...
            canonical_entity = self.db_manager.create_canonical_entity(canonical_entity_data)
