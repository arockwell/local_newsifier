...
def reset_db():
    ...
    if not confirm(
        "⚠️ DOUBLE CHECK: All existing data will be PERMANENTLY LOST! " "Type 'yes' to confirm"
    ):
        logger.info("Operation cancelled.")
        return
    ...
    try:
        ...
        with Session() as session:
            # Add essential data like RSS feeds - use direct SQL to avoid
            # import issues
            ...
            for url, name, description, is_active in feeds:
                stmt = text(
                    """
                    INSERT INTO rss_feeds (url, name, description, is_active,
                                        created_at, updated_at)
                    VALUES (:url, :name, :description, :is_active,
                           :created_at, :updated_at)
                """
                )
                session.execute(
