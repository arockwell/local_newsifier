...
Example usage:
    ...
    python demo_headline_trends.py --start-date 2023-01-01 --end-date 2023-03-31 \
        --interval month
...
def main():
    ...
    parser.add_argument(
        "--top", type=int, default=20, help="Number of top terms to include (default: 20)"
    )
    parser.add_argument(
        "--start-date", type=str, help="Start date for analysis (YYYY-MM-DD), overrides --days"
    )
    parser.add_argument(
        "--end-date",
        type=str,
        help=(
            "End date for analysis (YYYY-MM-DD), " "defaults to today if start-date is specified"
        ),
    )
    ...
    try:
        ...
        if args.start_date:
            ...
            results = flow.analyze_date_range(
                start_date=start_date, end_date=end_date, interval=args.interval, top_n=args.top
            )
        else:
            print(f"Analyzing headlines for the past {args.days} days...")
            results = flow.analyze_recent_trends(
                days_back=args.days, interval=args.interval, top_n=args.top
            )
