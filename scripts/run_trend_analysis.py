...
def main():
    ...
    # Run the flow
    state = flow.run_analysis(
        config=config, report_format=ReportFormat(args.format)
    )
    ...
    if state.detected_trends:
        ...
        for i, trend in enumerate(
            sorted(state.detected_trends, key=lambda t: t.confidence_score, reverse=True)[:5], 
            1
        ):
