...
def test_entity_default_values(db_session):
    ...
    entity = EntityDB(
        article_id=article.id, text="Gainesville", entity_type="GPE"
    )
    db_session.add(entity)
    db_session.commit()
...
def test_full_article_entity_workflow(db_session):
    ...
    article = ArticleDB(
        ...
        content=(
            ...
            "critical infrastructure projects for the University of Florida "
            "community."
        ),
    ...
    entities = [
        ...
        EntityDB(
            ...
            entity_type="PERSON",
            sentence_context=(
                "Mayor John Smith praised the decision, saying it would help "
        ...
        EntityDB(
            ...
            entity_type="ORG",
            sentence_context=(
                "critical infrastructure projects for the University of "
                "Florida community."
            ),
        ),
    ]
    ...
    # Verify article
    assert article.id is not None
    assert article.url == "https://example.com/news/8"
    assert article.title == "Local News: City Council Approves New Budget"
    assert article.source == "example.com"
    assert article.status == AnalysisStatus.ANALYSIS_SUCCEEDED.value
