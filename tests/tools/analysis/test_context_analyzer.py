...
class TestContextAnalyzer:
    ...
    def test_analyze_framing_leadership(
        self, context_analyzer, mock_spacy_model, leadership_tokens
    ):
    ...
    def test_analyze_context(
        self, context_analyzer, mock_spacy_model, positive_tokens, leadership_tokens
    ):
        ...
        context_analyzer.analyze_sentiment = Mock(
            return_value={
                "score": 0.5,
                "category": "positive",
                "positive_count": 3,
        ...
        context_analyzer.analyze_framing = Mock(
            return_value={
                "category": "leadership",
        ...
        result = context_analyzer.analyze_context(
            "This text has positive sentiment and leadership framing."
        )
    ...
    def test_analyze_entity_contexts(self, context_analyzer):
        ...
        context_analyzer.analyze_context = Mock(
            return_value={
        ...
        assert all(
            entity["context_analysis"]["sentiment"]["category"] == "positive" for entity in result
        )
        assert all(
            entity["context_analysis"]["framing"]["category"] == "leadership" for entity in result
        )
