...
from local_newsifier.crud.entity_profile import CRUDEntityProfile, entity_profile as entity_profile_crud
...
class TestEntityProfileCRUD:
    ...
    def test_create(self, db_session, create_canonical_entity, sample_entity_profile_data):
        ...
        db_profile = db_session.query(EntityProfileDB).filter(EntityProfileDB.id == profile.id).first()
        assert db_profile is not None
        assert db_profile.canonical_entity_id == obj_in.canonical_entity_id
        assert db_profile.profile_type == obj_in.profile_type

    def test_create_duplicate_fails(self, db_session, create_canonical_entity, sample_entity_profile_data):
        ...
        with pytest.raises(ValueError, match=f"Profile already exists for entity {create_canonical_entity.id}"):
            entity_profile_crud.create(db_session, obj_in=obj_in)
    ...
    def test_get_by_entity(self, db_session, create_canonical_entity, sample_entity_profile_data):
        ...
        profile = entity_profile_crud.get_by_entity(db_session, entity_id=create_canonical_entity.id)
    ...
    def test_update_or_create_update(self, db_session, create_canonical_entity, sample_entity_profile_data):
        ...
        db_updated = db_session.query(EntityProfileDB).filter(EntityProfileDB.id == db_profile.id).first()
    ...
    def test_update_or_create_create(self, db_session, create_canonical_entity):
        ...
        # Verify it was saved to the database
        db_profile = db_session.query(EntityProfileDB).filter(
