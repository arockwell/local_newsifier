...
def sample_config_data():
    """Sample data for creating a source configuration."""
    return {
...
class TestApifySourceConfigCRUD:
    ...
    def test_create(self, db_session: Session, sample_config_data):
        """Test creating a source configuration."""
        # Create the config
        created = apify_source_config.create(db_session, obj_in=sample_config_data)
        
        # Verify
        assert created.id is not None
        assert created.name == sample_config_data["name"]
        assert created.actor_id == sample_config_data["actor_id"]
        assert created.is_active is True
        assert created.schedule == sample_config_data["schedule"]
        assert created.source_type == sample_config_data["source_type"]
        assert created.source_url == sample_config_data["source_url"]
        assert created.input_configuration == sample_config_data["input_configuration"]

    def test_create_duplicate_name(self, db_session: Session, sample_config_data):
        """Test creating a source configuration with a duplicate name."""
        # Create the first config
        apify_source_config.create(db_session, obj_in=sample_config_data)
        ...
        assert f"Source configuration with name '{sample_config_data['name']}' already exists" in str(error)

    def test_get(self, db_session: Session, sample_config_data):
    ...
    def test_get_by_name(self, db_session: Session, sample_config_data):
    ...
    def test_get_by_actor_id(self, db_session: Session, sample_config_data):
        """Test getting source configurations by actor ID."""
        # Create the first config
        created1 = apify_source_config.create(db_session, obj_in=sample_config_data)
    ...
    def test_get_active_configs(self, db_session: Session, sample_config_data):
        """Test getting active configurations."""
        # Create an active config
        active_config = apify_source_config.create(db_session, obj_in=sample_config_data)
        
        # Create an inactive config
        inactive_data = sample_config_data.copy()
    ...
    def test_get_by_source_type(self, db_session: Session, sample_config_data):
        """Test getting configurations by source type."""
        # Create a news source
        news_config = apify_source_config.create(db_session, obj_in=sample_config_data)
        
        # Create a blog source
        blog_data = sample_config_data.copy()
    ...
    def test_get_scheduled_configs(self, db_session: Session, sample_config_data):
        """Test getting scheduled configurations."""
        # Create a scheduled config
        scheduled_config = apify_source_config.create(db_session, obj_in=sample_config_data)
        
        # Create an unscheduled config
        unscheduled_data = sample_config_data.copy()
    ...
    def test_update(self, db_session: Session, sample_config_data):
        ...
        assert updated.actor_id == sample_config_data["actor_id"]
        assert updated.source_type == sample_config_data["source_type"]

    def test_update_duplicate_name(self, db_session: Session, sample_config_data):
        """Test updating a source configuration with a duplicate name."""
        # Create the first config
        first_config = apify_source_config.create(db_session, obj_in=sample_config_data)
        
        # Create another config
        second_data = sample_config_data.copy()
    ...
    def test_update_last_run(self, db_session: Session, sample_config_data):
        """Test updating the last_run_at timestamp."""
        # Create the config
        created = apify_source_config.create(db_session, obj_in=sample_config_data)
        assert created.last_run_at is None
        
        # Update with a timestamp and verify it worked
        specific_date = datetime.now() - timedelta(days=1)  # Yesterday
        
        # Update the last_run_at
    ...
    def test_toggle_active(self, db_session: Session, sample_config_data):
    ...
    def test_remove(self, db_session: Session, sample_config_data):
