...
class TestEntityRelationshipCRUD:
    ...
    def test_create_or_update_create(
        self, db_session, create_canonical_entities, sample_entity_relationship_data
    ):
        """Test creating a new entity relationship with create_or_update."""
        # Ensure the entity IDs match the ones we created
        sample_entity_relationship_data["source_entity_id"] = create_canonical_entities[
            0
        ].id
        sample_entity_relationship_data["target_entity_id"] = create_canonical_entities[
            1
        ].id
        ...
        db_relationship = (
            db_session.query(entity_relationships)
            .filter(
