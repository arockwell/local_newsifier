...
class TestCRUDBase:
    ...
    def test_create_from_dict(self, db_session, sample_article_data):
        ...
        db_article = db_session.query(ArticleDB).filter(ArticleDB.id == article.id).first()
    ...
    def test_update(self, db_session, create_article):
        ...
        db_obj = db_session.query(ArticleDB).filter(ArticleDB.id == create_article.id).first()
        ...
        # Verify it was saved to the database
        db_article = db_session.query(ArticleDB).filter(ArticleDB.id == create_article.id).first()
