...
from local_newsifier.crud.canonical_entity import CRUDCanonicalEntity, canonical_entity as canonical_entity_crud
from local_newsifier.models.entity_tracking import (
    CanonicalEntityDB, CanonicalEntity, CanonicalEntityCreate, entity_mentions
)
...
class TestCanonicalEntityCRUD:
    ...
    def test_create(self, db_session, sample_canonical_entity_data):
        ...
        db_entity = db_session.query(CanonicalEntityDB).filter(CanonicalEntityDB.id == entity.id).first()
    ...
    def test_get_mentions_count(self, db_session, create_canonical_entity, create_entity):
        ...
        count = canonical_entity_crud.get_mentions_count(db_session, entity_id=create_canonical_entity.id)
    ...
    def test_get_entity_timeline(self, db_session, create_canonical_entity, create_article):
        ...
        assert any(entry["date"] == articles[0].published_at and entry["mention_count"] == 2 for entry in timeline)
        # The other articles should have 1 mention each
        for i in range(1, 5):
            assert any(entry["date"] == articles[i].published_at and entry["mention_count"] == 1 for entry in timeline)

    def test_get_articles_mentioning_entity(self, db_session, create_canonical_entity, create_article):
