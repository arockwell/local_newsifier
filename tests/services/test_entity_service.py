...
def test_process_articles_batch_partial_failure(event_loop_fixture):
    ...
    mock_entity_extractor.extract_entities.side_effect = [
        [{"text": "Test Entity", "type": "ORG", "context": "Test Entity is mentioned.", "start_char": 0, "end_char": 11}],
    ...
    # Assert
    assert result_state.status == TrackingStatus.SUCCESS  # Batch still succeeds with partial failures
    assert result_state.total_articles == 2
    assert result_state.processed_count == 2  # Both articles are processed, even though one has an error
...
def test_generate_entity_dashboard(event_loop_fixture):
    ...
    mock_canonical_entity_crud.get_by_type.return_value = [mock_canonical_entity1, mock_canonical_entity2]
    ...
    assert str(mock_canonical_entity1.name) in str(result_state.dashboard_data["entities"][0]["name"])
...
def test_find_entity_relationships(event_loop_fixture):
    ...
    mock_canonical_entity_crud.get_articles_mentioning_entity.return_value = [mock_article1, mock_article2]
    ...
    assert result_state.relationship_data["relationships"][0]["entity_name"] == mock_canonical_entity1.name
    ...
    assert result_state.relationship_data["relationships"][1]["entity_name"] == mock_canonical_entity2.name
