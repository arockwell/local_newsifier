...
def mock_config_crud():
    ...
    mock_crud.get.side_effect = lambda session, id: config1 if id == 1 else config2 if id == 2 else None
...
def test_create_schedule_for_config(schedule_manager, mock_apify_service, mock_config_crud, mock_session):
...
def test_update_schedule_for_config(schedule_manager, mock_apify_service, mock_config_crud, mock_session):
...
def test_delete_schedule_for_config(schedule_manager, mock_apify_service, mock_config_crud, mock_session):
...
def test_verify_schedule_status(schedule_manager, mock_apify_service, mock_config_crud, mock_session):
...
def test_sync_schedules(schedule_manager, mock_apify_service, mock_config_crud, mock_session):
    ...
    with patch.object(schedule_manager, 'create_schedule_for_config', return_value=True) as mock_create:
        with patch.object(schedule_manager, 'update_schedule_for_config', return_value=True) as mock_update:
            with patch.object(schedule_manager, '_clean_orphaned_schedules', return_value=1) as mock_clean:
...
def test_clean_orphaned_schedules(schedule_manager, mock_apify_service, mock_config_crud, mock_session):
