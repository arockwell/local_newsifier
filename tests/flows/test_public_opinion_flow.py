...
class TestPublicOpinionFlow:
    ...
    def flow(self, mock_session):
        ...
        with patch(
            "local_newsifier.flows.public_opinion_flow.SentimentAnalyzer"
        ) as mock_analyzer, patch(
            "local_newsifier.flows.public_opinion_flow.SentimentTracker"
        ) as mock_tracker, patch(
            "local_newsifier.flows.public_opinion_flow.OpinionVisualizerTool"
        ) as mock_visualizer:
    ...
    def test_init_without_session(self):
        ...
        with patch(
            "local_newsifier.database.engine.get_session", return_value=mock_context_manager
        ), patch("local_newsifier.flows.public_opinion_flow.SentimentAnalyzer"), patch(
            "local_newsifier.flows.public_opinion_flow.SentimentTracker"
        ), patch(
            "local_newsifier.flows.public_opinion_flow.OpinionVisualizerTool"
        ), patch(
            "local_newsifier.tools.sentiment_analyzer.spacy.load", return_value=MagicMock()
        ), patch(
            "fastapi_injectable.concurrency.run_coroutine_sync", return_value=None
        ):
    ...
    def test_analyze_articles_with_ids(self, flow):
        ...
        with patch("local_newsifier.crud.article.article.get_by_status"), patch(
            "local_newsifier.crud.article.article.update_status"
        ):
    ...
    def test_analyze_articles_without_ids(self, flow):
        ...
        with patch(
            "local_newsifier.crud.article.article.get_by_status",
    ...
    def test_analyze_topic_sentiment(self, flow):
        ...
        with patch("fastapi_injectable.concurrency.run_coroutine_sync", return_value=None), patch(
            "local_newsifier.database.engine.get_engine", return_value=MagicMock()
        ), patch("sqlalchemy.orm.Session", return_value=mock_sess):

            # Call method
            result = flow.analyze_topic_sentiment(topics=["climate"], days_back=7, interval="day")
    ...
    def test_analyze_entity_sentiment(self, flow):
        ...
        with patch("fastapi_injectable.concurrency.run_coroutine_sync", return_value=None), patch(
            ...
            result = flow.analyze_entity_sentiment(
                entity_names=["John Smith", "ABC Corp"], days_back=7, interval="day"
            )
    ...
    def test_detect_opinion_shifts(self, flow):
        ...
        with patch("fastapi_injectable.concurrency.run_coroutine_sync", return_value=None), patch(
            ...
            result = flow.detect_opinion_shifts(
                topics=["climate", "energy"], days_back=7, interval="day"
            )
    ...
    def test_correlate_topics(self, flow):
        ...
        flow.sentiment_tracker.calculate_topic_correlation.side_effect = [
            {"topic1": "climate", "topic2": "energy", "correlation": -0.8, "period_count": 7},
            {"topic1": "economy", "topic2": "politics", "correlation": 0.6, "period_count": 7},
        ]
        ...
        with patch("fastapi_injectable.concurrency.run_coroutine_sync", return_value=None), patch(
            ...
            result = flow.correlate_topics(
                topic_pairs=[("climate", "energy"), ("economy", "politics")], days_back=7
            )
    ...
    def test_generate_topic_report(self, flow):
        ...
        with patch("fastapi_injectable.concurrency.run_coroutine_sync", return_value=None), patch(
            ...
            md_result = flow.generate_topic_report(
                topic="climate", days_back=7, format_type="markdown"
            )

        assert md_result == "# Markdown Report"
        flow.opinion_visualizer.generate_markdown_report.assert_called_once_with(
            mock_viz_data, report_type="timeline"
        )
        ...
        with patch("fastapi_injectable.concurrency.run_coroutine_sync", return_value=None), patch(
            ...
            html_result = flow.generate_topic_report(
                topic="climate", days_back=7, format_type="html"
            )

        assert html_result == "<html>HTML Report</html>"
        flow.opinion_visualizer.generate_html_report.assert_called_once_with(
            mock_viz_data, report_type="timeline"
        )
        ...
        with patch("fastapi_injectable.concurrency.run_coroutine_sync", return_value=None), patch(
            ...
            text_result = flow.generate_topic_report(
                topic="climate", days_back=7, format_type="text"
            )

        assert text_result == "Text Report"
        flow.opinion_visualizer.generate_text_report.assert_called_once_with(
            mock_viz_data, report_type="timeline"
        )
    ...
    def test_generate_comparison_report(self, flow):
        ...
        with patch("fastapi_injectable.concurrency.run_coroutine_sync", return_value=None), patch(
            ...
            result = flow.generate_comparison_report(
                topics=["climate", "energy"], days_back=7, format_type="markdown"
            )
    ...
    def test_generate_report_with_error(self, flow):
        ...
        flow.opinion_visualizer.generate_markdown_report.assert_called_with(
    ...
    def test_error_handling_in_prepare_comparison_data(self, flow):
        ...
        with patch("fastapi_injectable.concurrency.run_coroutine_sync", return_value=None), patch(
            ...
            # Call method
            result = flow.generate_comparison_report(
                topics=["climate", "energy"], format_type="markdown"
            )
