...
def test_feeds_list_with_injectable(sample_feed):
    ...
    with patch(
        "local_newsifier.cli.commands.feeds.get_rss_feed_service", return_value=mock_rss_service
    ):
        # Run command
        runner = CliRunner()
...
def test_feeds_process_with_injectable(sample_feed):
    ...
    with patch(
        ...
        "local_newsifier.cli.commands.feeds.get_article_crud", return_value=mock_article_crud
    ), patch(
        "local_newsifier.cli.commands.feeds.get_news_pipeline_flow", return_value=mock_news_flow
    ), patch(
        "local_newsifier.cli.commands.feeds.get_entity_tracking_flow", return_value=mock_entity_flow
    ), patch(
        "local_newsifier.cli.commands.feeds.get_session"
    ):
